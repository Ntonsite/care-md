...
#        global $_POST;
    ...
    function SearchSelect($searchkey = '', $maxcount = 100, $offset = 0, $oitem = 'name_last', $odir = 'ASC', $fname = FALSE, $ctc_no = FALSE) {
        ...
        if (defined('SHOW_CTC_NO_CONTROLLER') && SHOW_CTC_NO_CONTROLLER) {       //Search by CTC ID if checked
            if ($ctc_no) {
                $this->sql = 'SELECT a.ctc_id, p.pid, p.selian_pid, p.name_2, p.name_last, p.name_first, p.date_birth, p.addr_zip, p.sex, p.death_date, p.status FROM care_person p, care_tz_arv_registration a '
                        . ' WHERE a.pid = p.pid '
