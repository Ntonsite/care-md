...
class Diagnostics extends Encounter {
  ...
  function displaynames($pid){
  	  ...
  	  $this->sql="SELECT name_first,name_2,name_last FROM care_person WHERE pid=$pid";
  	  $rs= $db->Execute($this->sql);
  	  while($rows = $rs->FetchRow()) {
  	  	   echo "<strong>First Name:</strong> ". $rows['name_first']. " <strong> Second Name:</strong>  ".$rows['name_2']."<strong> Last Name: </strong> ".$rows['name_last'];
  	  	}
